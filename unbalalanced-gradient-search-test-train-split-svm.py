...
# For each trial X (X=0,1,...,NUM_RUNS), training-datasetX will be used for training purpose and test-datasetX will
# be used for testing
...
for i in range(0,NUM_RUNS):
   ...
   # print(X_new_training.shape)
    ...
    classifier = OneVsRestClassifier(svm.SVC(random_state=seed))
    ...
    parameters_grid = {
        "estimator__C": [1,2,4,8], #Penalty parameter C of the error term
        "estimator__kernel": ["rbf","linear"], # different type of kernels to be explored
        "estimator__gamma":[0.001,0.01,0.1,0.75]
    }
